...
const CLUSTERS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L'] as const;
...
type BookingsMap = Record<ClusterType, Booking | null>;
...
export default function ClusterBookingPage() {
  ...
  const getBookingForSlot = useCallback((date: string, timeSlot: string): ClusterType | null => {
    ...
    return entry ? (entry[0] as ClusterType) : null;
  }, [bookings]);

  // Handle slot selection
  const handleSlotClick = (timeSlot: string) => {
    if (!selectedCluster) {
      alert('Please select a Cluster from the dropdown first!');
      return;
    }
      ...
      <div className="pixel-border-sm" style={{
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <select
            ...
            onChange={(e) => {
              setSelectedCluster(e.target.value as ClusterType | '');
              setSelectedSlot(null);
            }}
          >
        ...
        {selectedCluster && (
          <div style={{
            ...
            fontFamily: 'var(--font-comic)',
            fontSize: '15px',
            color: activeClusterBooking ? '#2E7D32' : '#C62828',
            fontWeight: 'bold',
            display: 'flex',
            alignItems: 'center',
            gap: '6px'
          }}>
            {activeClusterBooking ? (
              <span>
                ✅ Cluster {selectedCluster} is booked on{' '}
                <strong>
              ...
              <span>⚠️ Cluster {selectedCluster} does not have any booking yet. Choose an open slot below!</span>
            )}
          </div>
        )}
      </div>
        ...
        {!selectedCluster && (
          <div style={{
            ...
            <p style={{ fontSize: '15px', color: 'var(--text-secondary)', marginTop: '6px' }}>
              Choose Cluster A to L from the dropdown above to view and reserve slots.
            </p>
          </div>
        )}
              ...
              >
                ...
                <div style={{
                  ...
                  {isMine ? (
                    ...
                    <span style={{ color: '#b71c1c' }}>CLUSTER {bookedCluster}</span>
                  ) : (
          ...
          <table style={{
            ...
            <thead>
              <tr style={{ borderBottom: '3px solid var(--border-color)', background: '#f5f5f5' }}>
                <th style={{ padding: '10px', borderRight: '1px solid #ddd' }}>Cluster</th>
                ...
                return (
                  <tr 
                    ...
                    style={{ 
                      ...
                      background: selectedCluster === c ? '#e8f5e9' : 'transparent',
                      fontWeight: selectedCluster === c ? 'bold' : 'normal'
                    }}
                  >
                    <td style={{ padding: '8px 10px', borderRight: '1px solid #eee' }}>Cluster {c}</td>
            ...
            {isSuccess ? (
              <div style={{ textAlign: 'center', padding: '16px' }} className="success-pop">
                ...
                <p style={{ fontFamily: 'var(--font-comic)', fontSize: '15px', marginTop: '10px', color: '#555' }}>
                  Cluster <strong>{selectedCluster}</strong> booked for:<br />
              ...
              <>
                ...
                <div style={{
                  ...
                  You are booking this slot for <strong>Cluster {selectedCluster}</strong>.
