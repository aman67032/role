...
const AARAMBH_DATES: { value: string, label: string, day: string, blocked?: boolean }[] = [
  { value: '2026-04-30', label: 'APR 30', day: 'THU' },
  { value: '2026-05-01', label: 'MAY 01', day: 'FRI' },
];
...
export default function AarambhPage() {
  ...
  const [selectedDate, setSelectedDate] = useState(AARAMBH_DATES[0].value);
  ...
  useEffect(() => {
    fetchSlots();
    // Minimum 2 seconds loading for the initial mount (reduced from 5s for better UX unless requested)
  ...
  const isSelectedDateBlocked = AARAMBH_DATES.find(d => d.value === selectedDate)?.blocked;
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '16px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '28px', paddingTop: '16px' }}>
        ...
        <div className="speech-bubble" style={{ display: 'inline-block', marginTop: '14px' }}>
          <p style={{ fontFamily: 'var(--font-comic)', fontSize: '16px', lineHeight: '1.4', letterSpacing: '0.5px' }}>
            Team & Cohort Leaders, book your 15-min slot!
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        ★ April 30 – May 1, 2026 ★ Discussion Slots ★
      </div>
              ...
              <>
                <div style={{ marginBottom: '18px' }}>
                  <div className="pixel-border-sm" style={{
                    ...
                    {selectedSlot && SLOT_LABELS[selectedSlot.timeSlot]} • {selectedDate}
                  </div>
